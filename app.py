...
def mobile_api_required(fn):
    @wraps(fn)
    def wrapper(*args, **kwargs):
        expected = MOBILE_API_TOKEN or CRON_TOKEN
        if not expected:
            return jsonify({"ok": False, "error": "MOBILE_API_TOKEN not set"}), 400
...
def get_or_create_mobile_user(email):
...
def serialize_item_for_mobile(item_row, include_history=True):
    conn = db_conn()
    cur = conn.cursor()

    cur.execute(
        """
        SELECT ts, item_name, price_text, price_value, coupon_text, discount_percent, error
        FROM price_history
        WHERE item_id=%s
        ORDER BY ts DESC
        LIMIT 1
    ...
    if latest:
        try:
            latest_price_value = float(latest["price_value"]) if latest.get("price_value") is not None else None
        except Exception:
            latest_price_value = None

        try:
            discount_percent = int(float(latest["discount_percent"])) if latest.get("discount_percent") is not None else None
        except Exception:
            discount_percent = None

        coupon_text = latest.get("coupon_text")
        last_error = latest.get("error")

    try:
        target_value = float(item_row["target_price_value"]) if item_row.get("target_price_value") is not None else 0.0
    except Exception:
        target_value = 0.0
    ...
    return {
        ...
        "target_price_value": target_value,
        "created_at": item_row.get("created_at"),
        "title": latest.get("item_name") if latest else None,
        "current_price_text": latest.get("price_text") if latest else None,
        "current_price_value": latest_price_value,
        "discount_percent": discount_percent,
        "coupon_text": coupon_text,
        "coupon_percents": all_percents_from_text(coupon_text),
        "seller_name": None,
        "last_checked_at": latest.get("ts") if latest else None,
        "last_error": last_error,
        "history": history,
    }


def list_mobile_items(user_id):
    conn = db_conn()
    cur = conn.cursor()
    cur.execute(
        "SELECT * FROM items WHERE user_id=%s ORDER BY created_at DESC, id DESC",
        (user_id,),
    )
    item_rows = cur.fetchall() or []
    conn.close()
    return [serialize_item_for_mobile(r, include_history=True) for r in item_rows]
...
def run_global_scrape():
    ...
    try:
        interval_sec = int(get_setting("update_interval", "3600") or "3600")
    except Exception:
        interval_sec = 1800
...
@app.route("/api/mobile/bootstrap", methods=["POST"])
@mobile_api_required
def api_mobile_bootstrap():
    ...
    try:
        user = get_or_create_mobile_user(email)
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500

    try:
        interval_seconds = int(get_setting("update_interval", "3600") or "3600")
    except Exception:
    ...
    return jsonify(
        {
            ...
            "last_global_run": get_setting("last_global_run", None),
        }
    )


@app.route("/api/mobile/items", methods=["GET"])
@mobile_api_required
def api_mobile_items():
    user_id = parse_user_id_from_request()
    if not user_id:
        return jsonify({"ok": False, "error": "user_id is required"}), 400

    try:
        items = list_mobile_items(user_id)
        interval_seconds = int(get_setting("update_interval", "3600") or "3600")
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
    ...
    return jsonify(
        {
            "ok": True,
            "items": items,
            "update_interval_seconds": max(300, interval_seconds),
            "last_global_run": get_setting("last_global_run", None),
        }
    )


@app.route("/api/mobile/items", methods=["POST"])
@mobile_api_required
def api_mobile_add_item():
    ...
    if target not in (None, ""):
        try:
            ...
            return jsonify({"ok": False, "error": "target_price_value must be numeric"}), 400

    conn = db_conn()
    cur = conn.cursor()
    cur.execute("SELECT * FROM items WHERE user_id=%s AND asin=%s", (user_id, asin))
    existing = cur.fetchone()

    if existing:
        if target_value is not None:
            cur.execute(
                "UPDATE items SET target_price_value=%s, url=%s WHERE id=%s",
                (target_value, url, existing["id"]),
            )
            conn.commit()
            cur.execute("SELECT * FROM items WHERE id=%s", (existing["id"],))
            existing = cur.fetchone()
        conn.close()
        return jsonify({"ok": True, "item": serialize_item_for_mobile(existing, include_history=True), "created": False})

    cur.execute(
        "INSERT INTO items(user_id, asin, url, created_at, target_price_value) VALUES(%s,%s,%s,%s,%s) RETURNING *",
        (user_id, asin, url, now_utc_str(), target_value),
    )
    item = cur.fetchone()
    conn.commit()
    conn.close()
    ...
    try:
        data_row = run_async(scrape_one_amazon_sa, url)
        write_history(item["id"], data_row)
    except Exception:
        pass

    return jsonify({"ok": True, "item": serialize_item_for_mobile(item, include_history=True), "created": True})


@app.route("/api/mobile/items/<int:item_id>/target", methods=["PATCH"])
@mobile_api_required
def api_mobile_update_target(item_id):
    ...
    cur.execute("UPDATE items SET target_price_value=%s WHERE id=%s", (target_value, item_id))
    conn.commit()
    cur.execute("SELECT * FROM items WHERE id=%s", (item_id,))
    item = cur.fetchone()
    conn.close()

    return jsonify({"ok": True, "item": serialize_item_for_mobile(item, include_history=True)})


@app.route("/api/mobile/items/<int:item_id>", methods=["DELETE"])
@mobile_api_required
def api_mobile_delete_item(item_id):
    user_id = parse_user_id_from_request()
    if not user_id:
        return jsonify({"ok": False, "error": "user_id is required"}), 400
    ...
    conn = db_conn()
    cur = conn.cursor()
    cur.execute("SELECT id FROM items WHERE id=%s AND user_id=%s", (item_id, user_id))
    item = cur.fetchone()
    if not item:
        conn.close()
        return jsonify({"ok": False, "error": "item not found"}), 404
    ...
    conn.commit()
    conn.close()
    return jsonify({"ok": True, "deleted": True, "item_id": item_id})


@app.route("/api/mobile/items/<int:item_id>/check", methods=["POST"])
@mobile_api_required
def api_mobile_check_item(item_id):
    ...
    try:
        data_row = run_async(scrape_one_amazon_sa, item.get("url") or item.get("asin"))
        write_history(item["id"], data_row)
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
...
@app.route("/api/mobile/check-all", methods=["POST"])
@mobile_api_required
def api_mobile_check_all():
    ...
    conn = db_conn()
    cur = conn.cursor()
    cur.execute("SELECT id, asin, url FROM items WHERE user_id=%s ORDER BY created_at DESC", (user_id,))
    rows = cur.fetchall() or []
    conn.close()
    ...
    for row in rows:
        try:
            data_row = run_async(scrape_one_amazon_sa, row.get("url") or row.get("asin"))
            write_history(row["id"], data_row)
            updated += 1
        except Exception:
            errors += 1
